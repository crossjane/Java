...
public class _03_String02 {
	...
	public static void main(String[] args) {
		...
		// 포함되어 있으면 해당 문자의 인덱스 찾아서 리턴, 포함되어 있지 않으면 -1리턴.
		...
		System.out.println("str3에서 index 4부터 a의 index는 " + str3.indexOf('a',4));  //인덱스 4번부터 검사. 해서 처음 찾는 인덱스 값이 나옴.
		
		
		System.out.println("str3에서 abc의 index는 " + str3.indexOf("abc"));
		System.out.println("str2에서 abc의 index는 " + str2.indexOf("abc")); //포함하지 않는 값은 -1 리턴.
		...
		//배열에서 length가 메소드가 아니고 속성 값(멤버변수)이다.
		...
		//4. replace, replaceAll : 문자열에서 주어진 첫번째 매개변수를 찾아서 두번째 매개변수로
		//치환하는 메소드, replace와 replaceAll 의 차이는 replaceAll은 정규표현식을 매개변수로 
		//전달할 수 있다는 점이다.
		...
		//5. 체이닝 기법 
		//메소드를 체인처럼 엮어서 사용하는 것을 체이닝 기법이라한다.
