...
//		//아반떼가 있으면 true가 되면서 avanStream에 담김.
//		
//		Stream<HyundaiCar> avanStream = hCarStream.filter(hCar -> hCar.getModel().equals("아반떼"));
//		
//		//mapToInt(): int만 모여있는 스트림으로 변환
//		IntStream priceStream = avanStream.mapToInt(avante -> avante.getPrice());
//		
//		//2.최종처리
////		List<HyundaiCar> avanteList = avanStream.toList();
//		
////		System.out.println(avanteList);
//		int priceSum = priceStream.sum();
//		System.out.println("아반뗴 모델의 총 가격은" + priceSum);
		...
		int priceSum = hCarList.stream()	//Stream<HyundaiCar>
						// 모델명이 아반떼인 객체만 모여있는 Stream<HyundaiCar>
								...
								//모델명이 아반떼인 차의 가격만 모여 있는 IntStream
