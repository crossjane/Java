...
public class _02_ScopeOfVariables {
	...
	//인스턴스 변수 (static 키워드 없이 클래스안에서 선언된 변수)
	int num1;
	...
	public static void main(String[] args) {
		...
		if(sum == 0) {
			int ifNum  = 30; 
			//mainMethodNum는 if문 중괄호 블록보다 상위인 메소드 중괄호 블로겡서 선언되었기 때문에 
			//if문 안에서 사용가능하다.
			sum = mainMethodNum + ifNum;
		}
		
			//제어문에서 선언된 지역변수를 제어문 밖에서 사용하면 에러가 발생한다. 
			//  System.out.println(ifNum); error
...
//		  System.out.println(localResult);
	...
	public static void getSum() {
		//2-1. main 메소드에 선언된 변수를 다른 메소드에서 호출하면 에러가 발생한다.
		 //System.out.println(mainMethodNum); error
