...
public class _07_Wildcard {
	...
	public static void main(String[] args) {
		...
		// Long클래스는 Integer클래스와 같은 Number클래스를 상속받은 형제 클래스이지만 
		// Integer 클래스와 부모, 자식관계로 매핑되어 있지 않기 떄문에 super키워드로 하한제한을 건
		// 매개변수로는 사용할 수 없다. 
	...
	//제네릭 메소드의 리턴타입이나 매개변수에 타입매개변수의 지정 없이 제네릭으로 사용할 수 있다. 
	//와일드카드는 타입매개변수와는 다르게 단독으로는 사용 못하고 제네릭 클래스로 정의된 클래스의 
	//서브클래스 타입으로 사용할 수 있다.
	
//	public static List<?> createList(List<?> wildCardList){
//		
//		
//	}
	...
	//<? extends 부모클래스> : 부모클래스를 상속받은 클래스만 매개변수나 리턴타입으로 지정할 수 있고 
	//						매개변수나 리턴타입으로 지정할 수 있다.
	
	public static int getMaxList(List<? extends Number> wildCardList) {
		int max = (int)wildCardList.get(0).intValue();
	...
	//3. 와일드카드<?>는 super키워드로 하한제한을 걸 수 있다. 
	...
	//항상 지정해야 하지만 와일드카드<?> 타입매개변수를 지정 안해도된다. 
	//4-3. 제네릭 타입매개변수<T, K, V, E,...>는 단독으로 사용가능(타입매개변수 타입의 변수선언, 리턴타입, 매개변수)
	//와일드카드는 단독으로 사용할 수 없고 제네릭 클래스의 서브클래스 타입으로만 지정가능하다.
	//와일드카드는 제네릭클래스의 확장성을 한단계 더 높여 사용을 더 유연하게 해주는 기능을 한다.
	...
	public static List<?> copyList(List<?> wildCardList){
		...
		// 와일드카드는 향상된 for문도 사용 불가능 
		for(int i = 0; i < wildCardList.size(); i++) {
			returnList.add(String.valueOf(wildCardList.get(i)));
		}
