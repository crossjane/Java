...
public class _03_JDBC_Insert {
	...
	public static void main(String[] args) {
		...
		//PreparedStatement 는 쿼리에 파라미터를 추가할 수 있다. 
		...
		try {
			...
			//물음표(?)를 이용해서 파라미터 지정 
			String insertStudnet = "INSERT INTO STUDENT"
					+ " VALUES(?, ?, ?, ?, ?, ?)";
			
			//Statement 객체는 Connection 의 createStatement() 메소드를 통해 생성
			//PreparedStatement 객체는 Connection의 preparedStatement()메소드를 통해서 생성
			
			stmt = conn.prepareStatement(insertStudnet);
