...
// 인터페이스는 public static final로 선언된 상수와 추상메소드만 가질수 있다.
...
// 인터페이스도 추상클래스의 일종이기 때문에 객체를 만들 수 가 없어서 자식 객체를 사용한다.
...
public interface Board {
	...
	// static 키워드에 의해서 메모리에 저장될 때 값이 저장이 되어있지 않으면 안되기 때문에 
	// 선언 시 값을 지정하지 않으면 에러가 발생한다.
	int num = 10;
	
	//인터페이스에서 정의되는 모든 메소드는 추상메소드이기 때문에 
	//구현부{} 블럭을 만들면 에러가 발생한다.
...
//	}
	...
	// 인터페이스에 기능을 추가하면 상속받은 모든 자식클래스를 수정해야 되는 문제가 발생한다.
	// 이러한 문제를 해결하기 위해 나온 문법이 default 메소드
	// default 메소드는 인터페이스의 메소드에 구현부{}까지 만들 수 있는 메소드
